...
unordered_map<int, string> umapNum_ckt1, umapName_ckt1, umapNum_ckt2, umapName_ckt2; // umap[real CNF_VAR] -> real input/output port name
...
void printMatrix(const SatSolver& s, vector<vector<variable*>> const &M){
   for (int j = 0; j < M.size(); j++){
      for (int i=0; i < M[0].size(); i++){
         cout << M[j][i]->getName() << " " << M[j][i]->getSub1() << " " << M[j][i]->getSub2() << " ";
         cout << M[j][i]->getVar() << " " << M[j][i]->getVar2() << "\tAssign: ";
...
void printArr(const SatSolver& s, vector<variable*> const &Arr){
   for (int i = 0; i< Arr.size(); i++){
      cout << Arr[i]->getName() << " " << Arr[i]->getSub1() << " " << Arr[i]->getSub2() << " ";
...
void constraint5_miter (SatSolver& s){ // for mitersolver, aji -> xi == yj;  bji -> xi != yj;  aj MI+1 -> 0 == yj; bj MI+1 -> 1 == yj;  
   for (int j = 0; j < MI[0].size(); j++){
      ...
      // constant 0 binding
      Lit a_const = ~Lit(MI[MI.size()-2][j]->getVar2());  lit_const.push(a_const);
      Lit y_neg   = ~Lit(y[j]->getVar2());                lit_const.push(y_neg); 
      s.addClause(lit_const); lit_const.clear();

      // constant 1 binding
      Lit b_const = ~Lit(MI[MI.size()-1][j]->getVar2());  lit_const.push(b_const);
      Lit y_pos   =  Lit(y[j]->getVar2());                lit_const.push(y_pos); 
      s.addClause(lit_const); lit_const.clear();

      for (int i = 0; i < MI.size()-2; i++){
         if(i%2 == 0){
            vec<Lit> lits;
            Lit ab = ~Lit(MI[i][j]->getVar2());  lits.push(ab);
            Lit xv =  Lit(x[i/2]->getVar2());    lits.push(xv);
            Lit yv = ~Lit(y[j]->getVar2());    lits.push(yv);
            s.addClause(lits); lits.clear();
         
            lits.push(ab);
            xv = ~Lit(x[i/2]->getVar2());        lits.push(xv); 
            yv =  Lit(y[j]->getVar2());        lits.push(yv);
            s.addClause(lits); lits.clear();
         }
         else{
            vec<Lit> lits;
...
void initCircuit(SatSolver& s, SatSolver& s_miter){ //create 2D array ETs 
   ...
   s.addClause(lits); lits.clear(); // constraint 1.  sum(sum(c_ij + d_ij)) >0
   constraint2(s);
   constraint3(s);
   // solver(solver1 is used for determining feasible a, b, c, d (permutations), has nothing to do with x, y, f, g)

   constraint4_miter(s_miter);
   constraint5_miter(s_miter);   
}
void readAAG(){ // read aag file: top1.aag, top2.aag
...
void readMAP(){ // read map     : map1.txt, map2.txt
   ...
   while(fmap1 >> inout >> aagVar >> cnfVar){ 
      // inout: 2 -> input, 3 -> output. But portnum has inout info, don't need to store inout.
      // aagVar doesn't matter as well.
   ...
   while(true){
      ...
      if(tmp == "p"){
         ...
         while(fcnf1 >> litTmp){
            // cout << "LL: " << litTmp << endl;
            if(litTmp != 0){
               if(umapNum_ckt1.find(abs(litTmp)) == umapNum_ckt1.end()){ // doesn't exist
                  umapNum_ckt1[abs(litTmp)] = "n" + to_string(abs(litTmp));
                  ...
                  string portNumTmp = umapNum_ckt1[abs(litTmp)]; // i0, i1 ..  
                  // cout << "PPP: " << portNumTmp << endl;
                  // add clause to s_miter according to i/o and num. ex: i0 --->  x[0]->getVar2()
                  if(portNumTmp[0] == 'i'){
                     Lit li;
                     if(litTmp > 0)
                        li =  Lit(x[stoi(portNumTmp.substr(1))]->getVar2());
                     else 
                        li = ~Lit(x[stoi(portNumTmp.substr(1))]->getVar2());
                  
                     lits.push(li);
                     test.push_back(x[stoi(portNumTmp.substr(1))]->getVar2());
                  }
                  else if(portNumTmp[0] == 'o'){ // o
                     Lit li;
                     if(litTmp > 0)
                        li =  Lit(f[stoi(portNumTmp.substr(1))]->getVar2());
                     else 
                        li = ~Lit(f[stoi(portNumTmp.substr(1))]->getVar2());
                  
                     lits.push(li);
                     test.push_back(f[stoi(portNumTmp.substr(1))]->getVar2());
                  }
               ...
               s_miter.addClause(lits);  lits.clear();
   ...
   while(true){
      ...
      if(tmp == "p"){
         ...
         while(fcnf2 >> litTmp){
            // cout << "LL: " << litTmp << endl;
            if(litTmp != 0){
               if(umapNum_ckt2.find(abs(litTmp)) == umapNum_ckt2.end()){ // doesn't exist
                  umapNum_ckt2[abs(litTmp)] = "n" + to_string(abs(litTmp));
                  Var nV = s_miter.newVar();
                  ...
                  string portNumTmp = umapNum_ckt2[abs(litTmp)]; // i0, i1 ..  
                  // cout << "PPP: " << portNumTmp << endl;
                  // add clause to s_miter according to i/o and num. ex: i0 --->  x[0]->getVar2()
                  if(portNumTmp[0] == 'i'){
                     Lit li;
                     if(litTmp > 0)
                        li =  Lit(y[stoi(portNumTmp.substr(1))]->getVar2());
                     else 
                        li = ~Lit(y[stoi(portNumTmp.substr(1))]->getVar2());
                     ...
                     Lit li;
                     if(litTmp > 0)
                        li =  Lit(g[stoi(portNumTmp.substr(1))]->getVar2());
                     else 
                        li = ~Lit(g[stoi(portNumTmp.substr(1))]->getVar2());
   ...
   for(int i=0; i < x.size(); i++){
      for(int j=0; j < y.size(); j++){
         ...
         if(s_miter.getValue(x[i]->getVar2()) == s_miter.getValue(y[j]->getVar2())){
            ...
            lits.push(Lit(MI[2*i][j]->getVar()));   // ex: x2 != y5 -> a2,5      x2@x[1]; y5@y[4]; b2,5@MI[8][1]
         }
      }
   }
...
void AddLearnedClause_const(SatSolver& s, SatSolver& s_miter){ // for const 0, 1
   ...
   for(int j=0; j < y.size(); j++){
      // cout << "xy" << endl;
      if(s_miter.getValue(y[j]->getVar2()) == 1){
         lits.push(Lit(MI[2*inputNum_ckt1][j]->getVar())); // ex: x2 == y5 -> b2,5      x2@x[1]; y5@y[4]; b2,5@MI[9][1]
      }
      else{
...
int main(){
   ...
   */
   ...
   // cout << inputNum_ckt1 << " " << outputNum_ckt1 << " " << inputNum_ckt2 << " " << outputNum_ckt2 << endl; 
