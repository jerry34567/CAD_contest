...
class SatMgr
{
    ...
    public:
                        ...
                        for (int j = 0; j < MI[0].size(); j++) {
                            if (solver.getValue(MI[i][j]->getVar()) == 1) {
                                ...
                                cout << "i: " << i << " j: " << j << " val: "
                                     ...
                                     << endl;
                            } else if (solver.getValue(MI[i][j]->getVar()) ==
                                       0) {
                                Lit v = ~Lit(MI[i][j]->getVar2());
                                assump.push(v);
                            }
                        ...
                        for (int j = 0; j < MO[0].size(); j++) {
                            if (i % 2 == 0) {
                                if ((solver.getValue(MO[i][j]->getVar()) +
                                     solver.getValue(MO[i + 1][j]->getVar())) ==
                                    0) {
                            ...
                            if (solver.getValue(MO[i][j]->getVar()) == 1) {
                                ...
                                cout << "i: " << i << " j: " << j << " val: "
                                     ...
                                     << endl;
                            } else if (solver.getValue(MO[i][j]->getVar()) ==
                                       0) {
                                Lit v = ~Lit(MO[i][j]->getVar2());
                                assump.push(v);
                            }
        ...
        void constraint4_miter(SatSolver& s) { // for mitersolver, intentionally
                                               // make cji -> fi != gj
            vec<Lit>    lit_vec;
            ...
            for (int j = 0; j < MO[0].size(); j++) {
                for (int i = 0; i < MO.size(); i = i + 2) {
                    ...
                    s.addXorCNF(temp1, f[i / 2]->getVar2(), false,
                                g[j]->getVar2(), false);
                    s.addAigCNF(temp2, temp1, true, MO[i][j]->getVar2(), false);
                    s.addAigCNF(temp3, temp1, false, MO[i + 1][j]->getVar2(),
                                false);
            ...
            for (int i = 0; i < f.size(); i++) {
                for (int j = 0; j < g.size(); j++) {
                    ...
                    if (s_miter.getValue(f[i]->getVar2()) ==
                        ...
                        temp_lits.push(
                            ~Lit(dV)); // ex: f2 == g5 -> not d2,5 f2@f[1];
                                       // g5@g[4]; d2,5@MI[9][1]
                        ...
                        temp_lits.push(
                            ~Lit(cV)); // ex: f2 != g5 -> not c2,5 f2@x[1];
                                       // g5@y[4]; c2,5@MI[8][1]
        ...
        void AddLearnedClause_const(SatSolver& s,
            ...
            for (int j = 0; j < y.size(); j++) {
                ...
                if (s_miter.getValue(y[j]->getVar2()) == 1) {
                    lits.push(
                        Lit(MI[2 * inputNum_ckt1][j]
                                ->getVar())); // ex: x2 == y5 -> b2,5 x2@x[1];
                                              // y5@y[4]; b2,5@MI[9][1]
                } else {
                    lits.push(
                        Lit(MI[2 * inputNum_ckt1 + 1][j]
                                ->getVar())); // ex: x2 != y5 -> a2,5 x2@x[1];
                                              // y5@y[4]; b2,5@MI[8][1]
                }
            }
