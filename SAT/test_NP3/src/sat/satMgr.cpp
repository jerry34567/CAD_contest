        ...
        for (int i = 0; i < M[0].size(); i++) {
            ...
            if (s.getValue(M[j][i]->getVar()) == 1 && IO == 1) {
                if (j == 2 * inputNum_ckt1) {
                    ...
                    cout << portname_ckt2[i] << " ==  " << portname_ckt1[j / 2]
                         << endl;
                ...
                if (j % 2 == 0) {
                    ...
                    cout << portname_ckt2[i + inputNum_ckt2] << " == !"
                         << portname_ckt1[j / 2 + inputNum_ckt1] << endl;
                    outputMatch.push_back(tuple<bool, Var, Var>(
...
bool
SatMgr::addBindClause(bool   isnegate, // only bind input
...
// void
// SatMgr::reportResult(const SatSolver& solver, bool result) {
//     solver.printStats();
//     cout << (result ? "SAT" : "UNSAT") << endl;
//     if (result) {
